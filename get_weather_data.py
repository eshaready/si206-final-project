...
def get_weather_data():
    ...
    response = requests.request("GET", "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/United%20States/2020-01-01/2020-12-31?unitGroup=metric&include=days&key=2AMFZFKRG88567QNEPA2AGNHT&contentType=csv")
    if response.status_code != 200:
        print('Unexpected Status code: ', response.status_code)
        sys.exit()

    CSVText = csv.reader(response.text.splitlines(), delimiter=',',quotechar='"')
    with open('2020.csv', 'w', newline='') as output_file:
